...
def render_report(path: Path, meta: Dict, results: Dict[str, TargetResult], findings: List[Finding], missing_optional: List[str]):
    ...
    for t, tr in results.items():
        ...
        tls_findings = [f for f in tr.findings if "tls" in " ".join(f.tags).lower() or "certificate" in f.title.lower()]
...
def main():
    ...
    if resume:
        ...
        for t, r in state["results"].items():
            ...
            tr.findings = [Finding(target=x["target"], title=x["title"], severity=x["severity"], description=x["description"], sources=set(x.get("sources", [])), evidence=x.get("evidence", []), cves=set(x.get("cves", [])), ports=set(x.get("ports", [])), protocols=set(x.get("protocols", [])), urls=set(x.get("urls", []))) for x in r.get("findings", [])]
            results[t] = tr
    else:
